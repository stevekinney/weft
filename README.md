...
Serverless hosts that cannot keep process-local intervals alive can set `backgroundTasks: 'manual'` and call `engine.runMaintenance()` from an alarm or Cron wake-up to drive timers, scheduled starts, update-response cleanup, retention, and alert evaluation.
