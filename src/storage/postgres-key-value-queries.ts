...
// `RETURNING key` makes the deleted-row count portable: a plain DELETE reports
// its count in a driver-specific field (`rowCount` on node-postgres, `affectedRows`
// on PGlite), but `RETURNING key` surfaces one row per deleted key so the caller
// can use `rows.length` uniformly across drivers.
export const PG_DELETE_KEYS_BY_PREFIX = 'DELETE FROM kv WHERE key >= $1 AND key < $2 RETURNING key';
...
export function buildPostgresKeyRangeDelete(
  ...
  // `RETURNING key` keeps the deleted-row count portable across drivers — see
  // the note on PG_DELETE_KEYS_BY_PREFIX.
  if (options.limit === undefined) {
    return { parameters, sql: `DELETE FROM kv WHERE ${whereClause} RETURNING key` };
  }

  parameters.push(options.limit);
  return {
    parameters,
    sql: `DELETE FROM kv WHERE key IN (SELECT key FROM kv WHERE ${whereClause} ORDER BY key ASC LIMIT $${parameters.length}) RETURNING key`,
  };
}
