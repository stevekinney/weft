...
describe('dispatchJsonRpc — batch size cap', () => {
  it('rejects a batch that exceeds maxBatchSize with InvalidRequest (-32600)', async () => {
    // Without this cap, a 1 MB body can pack thousands of tiny method
    // calls that each touch the engine. Since batches are sequential
    // (Track 8 decision 13), that's a DoS amplifier on any transport
    // adapter that accepts arbitrary-size bodies.
    const registry = createOperationRegistry([
      makeOp({
        name: 'weft.test.noop',
        inputSchema: z.object({}),
    ...
    const items = Array.from({ length: 5 }, (_, i) => ({
      jsonrpc: '2.0',
      method: 'weft.test.noop',
      id: i,
    }));
    const result = await dispatchJsonRpc(JSON.stringify(items), {
      ...baseContext(),
      registry,
      maxBatchSize: 3,
    });
    expect(result.kind).toBe('single');
    if (result.kind !== 'single') throw new Error('shape');
    if (!('error' in result.response)) throw new Error('expected error envelope');
    expect(result.response.error.code).toBe(-32600);
    expect(result.response.id).toBeNull();
  });
  ...
  it('applies the default cap (100) when maxBatchSize is omitted', async () => {
    const registry = createOperationRegistry([
      makeOp({
        name: 'weft.test.noop',
        inputSchema: z.object({}),
    ...
    const result = await dispatchJsonRpc(JSON.stringify(items), {
      ...baseContext(),
      registry,
      // maxBatchSize omitted — default is 100.
    });
    expect(result.kind).toBe('single');
    if (result.kind !== 'single') throw new Error('shape');
    if (!('error' in result.response)) throw new Error('expected error envelope');
