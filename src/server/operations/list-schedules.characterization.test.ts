 ...
 *   4. limit         — must be a positive integer when provided
 *   5. offset        — must be a non-negative integer when provided
 *
...
describe('list-schedules — validation precedence', () => {
  ...
  it('reports status error before limit error when both are invalid', async () => {
    ...
    const response = await handleRequest(
      getRequest('status=bad-status&limit=-1'),
      engine,
      { operationRegistry: registry, restBindings: bindings, ...apiKeyAuthContext() },
    );
  ...
  it('reports status error when status and limit and offset are all invalid', async () => {
    engine = createEngine();
    const response = await handleRequest(
      getRequest('status=invalid&limit=0&offset=-1'),
      engine,
      { operationRegistry: registry, restBindings: bindings, ...apiKeyAuthContext() },
    );
    expect(response.status).toBe(400);
