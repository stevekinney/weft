/**
 * `weft schedule <action>` argument parsing. Extracted from
 * `parse-arguments.ts` so both modules stay under the file-length limit.
 *
 * Only {@link parseScheduleArguments} is consumed externally (by the
 * subcommand-parser table); the remaining helpers are private to this module.
 *
 * @module cli/schedule-arguments
 */
...
/** Parse `weft schedule list|create|pause|resume|cancel` into a {@link CliCommand}. */
