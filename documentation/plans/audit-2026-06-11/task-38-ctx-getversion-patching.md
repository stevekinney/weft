...
Add ctx.getVersion(changeId: string, minSupported: number, maxSupported: number): WorkflowOperation<number>. On first execution it checkpoints the current maxSupported version under changeId. On replay it returns the checkpointed version (which may be older). The checkpointed version lives alongside the other accumulatedResults in the checkpoint locals. This allows: old workflows to keep following the old branch, new workflows to take the new branch, and a safe deprecation path (remove min-supported branch once no workflows on that version remain). Requires adding a new operation type 'get-version' to ContextOperationRequest and a corresponding handler in operations-activity.ts.
