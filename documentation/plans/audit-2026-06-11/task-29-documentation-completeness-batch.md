# Task 29: Documentation completeness batch (docs-only)

**Severity:** medium

## Finding: No dedicated error-codes reference page for WeftErrorCode union and FaultCode union
...
## Finding: HTTP long-poll transport is entirely undocumented at the protocol level — wrong endpoints in the remote-workers guide
...
## Finding: weft version:check command is experimental by README policy but promoted as a solved deployment-safety tool in temporal-comparison.md
...
Documentation-only. The long-poll item documents the EXISTING wire protocol (fixing the wrong endpoints in the remote-workers guide) — it does not change the protocol. Conformance-kit expansion belongs to the export-storage-conformance-kit task, not here.
