...
Hand-authored test TypeScript is also actionable when repeated setup obscures the one behavior each test proves. Prefer a narrow test-only helper that owns identical fixture construction, storage fault injection, timer-index deletion, warning capture, or resolver-settlement choreography while keeping scenario-specific timing inputs and assertions local to the tests. For example, the cleanup for issues #651 and #652 extracted shared lease-renewal and sleep-timer race fixtures without moving the before-expiry, after-expiry, stale-marker, or spurious-marker expectations out of their tests.
