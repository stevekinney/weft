...
**Loser results are abandoned.** Once a winner is selected, Weft stops driving the losing branch generators and discards their results. A losing inline activity receives an abort through `ActivityContext.signal`, but cancellation is cooperative: work and side effects can continue when the activity does not observe that signal. Worker-pooled activities do not receive this race-loss abort. Design race branches to be idempotent or pair them with compensation.
...
> By default, records go to the current process console. Install a host sink with [`EngineOptions.onLog`](configuration.md#engineoptions) to route every non-replayed inline record into your own logging stack (pino / winston / OpenTelemetry, etc.) instead — when a sink is set, the console is no longer also called, so there is no duplicate noise. The sink receives a `WorkflowLogRecord` (both `WorkflowLogRecord` and `WorkflowLogLevel` are exported for typing it) and is never invoked for records suppressed during replay. If the sink throws, Weft catches that logging failure, sends the record to the matching console method, and lets the workflow continue; `ctx.speculate()` branches use the same sink and fallback behavior. In worker-pool mode, records still go to the worker process's console; routing worker-mode records back to the host sink is tracked in [issue #529](https://github.com/stevekinney/weft/issues/529). Inline timestamps come from the engine clock; worker-mode timestamps come from the worker process wall clock.
