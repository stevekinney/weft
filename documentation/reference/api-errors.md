...
| `VersionMismatchError`                      | `VersionMismatchError`                      | Recovery sees a stored workflow version that differs from the registered workflow version.                                | Isolated to that one workflow by default (see [Version drift](../guides/recovery-and-deploys.md#version-drift-versionmismatchpolicy)); `recoverAll({ versionMismatchPolicy: 'throw' })` blocks the whole batch instead |
