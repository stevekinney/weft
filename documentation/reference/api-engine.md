...
Pass `options.onTerminalConflict: 'start-new'` when a stable workflow id should be reusable after the prior run reaches a terminal state. This requires `options.id`, requires `signal.signalId`, and rejects `options.idempotencyKey`: idempotency keys are permanent at-most-once mappings and cannot restart a terminal run. With the policy enabled, a terminal prior run is purged through the same terminal-replacement path as `engine.start(..., { onTerminalConflict: 'start-new' })`, then the fresh run is created and receives the initial signal in the create batch. Non-terminal targets are still signalled and never replaced. `WeftClient.startOrSignal()` forwards the same policy over `LocalClient` and `HttpClient`; the REST/JSON-RPC `weft.workflows.startorsignal` operation is marked destructive because this option can purge the prior terminal run.
