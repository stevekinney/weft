...
| `ownership`                       | `'none' \| 'lease'`        | `'none'`              | Deployment ownership posture. `'lease'` acquires a storage ownership lease before recovery for a clean rolling-deploy handoff AND fences every engine-owned durable write on the lease epoch, so a deposed instance loses its write (and halts) instead of corrupting a successor (requires `conditionalBatch`). A genuine single-writer correctness backstop. Starting work before the lease is held (e.g. `new Engine({ ownership: 'lease' })` then `start()` without `recoverAll()`) throws `EngineLeaseNotHeldError`. See the [singleton guide](../guides/singleton-service-deployment.md#optional-lease-based-ownership-for-a-clean-deploy-handoff). |
