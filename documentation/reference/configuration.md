...
| `ownership`                       | `'none' \| 'lease'`        | `'none'`              | Single-writer ownership posture. `'lease'` acquires a storage ownership lease before recovery for a clean rolling-deploy handoff (requires `conditionalBatch`). It is a deploy-ergonomics aid, not yet a correctness backstop. See the [singleton guide](../guides/singleton-service-deployment.md#optional-lease-based-ownership-for-a-clean-deploy-handoff). |
