...
`payloadSize.maxBytes` caps the codec-encoded byte length of each workflow input, signal payload, and activity result at admission time. A payload exactly at the limit is allowed; one byte over the limit throws `PayloadSizeExceededError` before any durable write. `0`, `null`, `undefined`, or omission disables the cap and the disabled path performs no extra encode. The cap is separate from storage compression and from Worker protocol message bounds. For worker WebSocket connections, `payloadSize.maxBytes` also informs the transport-layer frame-size limit, which is hard-capped at 4 MiB regardless of a higher `payloadSize.maxBytes`: a frame larger than the limit is rejected by the transport before any JSON parse occurs. A positive `payloadSize.maxBytes` below 4 MiB tightens the frame limit to that value (the minimum of the two is used); a disabled cap (`0`, `null`, `undefined`, or omission) leaves the frame limit at the 4 MiB ceiling rather than removing it. A single worker activity result therefore cannot exceed 4 MiB on the wire even when the admission cap is higher or disabled.
