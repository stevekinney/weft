...
When the transport runs **without authentication** (`authRequired: false`), every anonymous caller shares one identity, so the session id alone is not a credential — and it is returned on every response, where it can leak through logs or proxies. To isolate concurrent anonymous callers, `initialize` also returns a one-time `Mcp-Session-Token`: store it from the `initialize` response and send it as the `Mcp-Session-Token` header on every subsequent POST, GET, and DELETE for that session. The token is disclosed only on the `initialize` response and never echoed afterward, so a leaked session id alone cannot drive, read, or terminate another caller's session. Authenticated sessions already re-present their credential on each request and do not require this token; the session binding for authenticated callers is unchanged.
