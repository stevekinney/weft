...
> Durable finalizers work in both inline and worker execution modes — the teardown always runs on the engine host, even when `activityExecution` is configured separately, so you can register a `finalizer` freely on a worker-mode engine. The one caveat: `ctx.setFinalizerState` is unavailable inside a worker generator, so a worker-mode workflow that records teardown state must run in inline mode instead.

One thing the worker boundary changes: `ctx.setFinalizerState` is a workflow-context method, and the worker generator runs without access to it. If your finalizer needs recorded state, run that workflow in inline execution mode (the default). `ctx.onCancel` is inline-only for the same reason — a worker-mode workflow that needs best-effort in-process cleanup should do it in the workflow body (a `ctx.run` destroy step inside a `try/finally`).
