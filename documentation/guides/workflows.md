...
> [!NOTE] Trust contract: finalizer runs on the engine host
> Durable finalizers work in both inline and worker execution modes. Worker mode isolates the workflow generator in a Web Worker; the finalizer teardown runs on the engine host regardless of execution mode, and this holds even when `activityExecution` is configured separately. Register a `finalizer` freely on a worker-mode engine. Note also that `ctx.setFinalizerState` is not available inside the worker generator (`WorkerWorkflowContext` does not expose it). Record finalizer state from engine-host code — for example, write it to storage before cancelling — or switch to inline execution mode if you need to call `ctx.setFinalizerState` from within the workflow body. Note that `ctx.onCancel` is still inline-only — it is unavailable in worker mode — so a worker-mode workflow that needs best-effort in-process cleanup should do it in the workflow body (a `ctx.run` destroy step inside a `try/finally`).
