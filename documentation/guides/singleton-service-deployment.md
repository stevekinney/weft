...
When enabled, each engine writes a periodic heartbeat to the store and warns (via `process.emitWarning`) if it sees another instance's heartbeat advancing while it is also running. That is precisely the autoscaling-to-two or overlapping-deploy case.
...
A few properties worth knowing. It warns only when a foreign heartbeat advances across two of its own intervals, so a clean `Recreate` deploy (no overlap) and a brief drain overlap both stay quiet—only sustained overlap warns. The default heartbeat interval is `15s`; tune it with `secondInstanceHeartbeatInterval`, but keep it comfortably above both your deploy drain window and your hosts' clock skew, since the staleness window is derived from it. And because it writes to the store on every interval, it has an ongoing cost—leave it off unless you want the backstop. It is off by default.
