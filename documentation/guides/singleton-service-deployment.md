...
ADR 0002 also specifies three bounded-cardinality metrics—`weft_workflow_claim_attempts_total{outcome}`, `weft_workflow_claims_active`, and `weft_workflow_claim_renewal_failures_total`—and an on-demand per-workflow ownership status diagnostic. The engine records the three metrics internally today, but they are not yet wired into the server's Prometheus exporter, and the per-workflow diagnostic endpoint does not exist yet either; neither is available to alert on until that follow-up work lands. Until then, the two warning events above are your operator-facing signal for claim loss and stale-wake discards.
