...
> [!NOTE]
> "Singleton" here is a deployment shape, not a code pattern. It means exactly one engine process owns a given durable store at a time. It is the default, and remains the only supported model under `ownership: 'none'` or `ownership: 'lease'`. Fenced multi-process ownership is available as an opt-in alternative topology—see [When to choose `workflow-lease` over `lease`](#when-to-choose-workflow-lease-over-lease) below and [One engine per durable store](recovery-and-deploys.md#one-engine-per-durable-store).

## Why one instance

Recovery runs on boot by default: a fresh engine sweeps the store for in-flight workflows and resumes them. With one owner, that sweep is safe. Point two engines at the same store and recovery is uncoordinated—both can resume the same workflow and both can execute its next step, firing the next activity twice. By default (`ownership: 'none'`), there is no lock, lease, or fence preventing this, so the constraint lives entirely in your deployment configuration. Opting into `ownership: 'lease'` (below) adds a genuine engine-side lease-epoch fence, and because exactly one engine holds that lease for the whole store, it does prevent two _legitimate_ owners from running at once. Two limits remain. A deposed owner—one that stalled past its lease TTL while a successor took over—can still execute a workflow's next step in the window before its next durable write loses the epoch compare-and-swap; this bounded correctness caveat persists under any lease-style fence, `workflow-lease` included. And because the lease is global rather than per workflow, it serializes the entire store behind one writer instead of letting distinct workflows progress on distinct engines. [ADR 0002—Fenced Per-Workflow Ownership (MultiEngine)](../contributing/architecture-decisions/0002-multiengine-per-workflow-ownership.md) closes that second gap with `ownership: 'workflow-lease'`, a shipped alternative topology described in [When to choose `workflow-lease` over `lease`](#when-to-choose-workflow-lease-over-lease) below.
...
## Per-workflow ownership (`workflow-lease`)

`ownership: 'workflow-lease'` fences execution per workflow instead of fencing the whole store behind one writer, so multiple engine processes can share one durable store, each claiming a disjoint set of workflows. It is specified in [ADR 0002—Fenced Per-Workflow Ownership (MultiEngine)](../contributing/architecture-decisions/0002-multiengine-per-workflow-ownership.md) and is fully wired in: claims are acquired before any user code runs for a workflow—folded into starts, delayed-start fires, scheduled runs, child launches, bulk retry reactivation, and standalone resume at recovery—and every engine-owned workflow-lifecycle write is fenced on the claim's epoch the same way `ownership: 'lease'` fences on the global lease epoch.
...
await using storage = new NeonStorage({ url: process.env['NEON_DATABASE_URL']! });
await using engine = await Engine.create({ storage, ownership: 'workflow-lease' });
void engine;
```
...
### Tuning `workflowClaimTtl` and `workflowClaimRenewInterval`

Tuning (all optional, durations like `'30s'`): `workflowClaimTtl` (default `30s`) is how long a per-workflow claim stays valid without renewal; `workflowClaimRenewInterval` (default `5s`) is the heartbeat cadence—keep it well below the TTL. Construction enforces two mechanically checkable floors: `workflowClaimRenewInterval` must be strictly less than `workflowClaimTtl`, and `workflowClaimTtl` must be at least `3` (`WORKFLOW_CLAIM_TTL_SAFETY_MULTIPLIER`) times `workflowClaimRenewInterval`. The shipped defaults satisfy both with room to spare.

Those two checks catch incoherent configuration; they do not, on their own, size `workflowClaimTtl` correctly. Renewal is timer-driven, so an `await` on an in-flight activity does not block it—but the timer only fires when the event loop turns. A CPU-bound inline activity or a long synchronous section of user code blocks the loop and prevents the heartbeat from running at all, for as long as the stall lasts. Under a stall the claim can expire and be taken over even though no heartbeat merely slipped a cycle—**`workflowClaimTtl` must therefore be sized above your deployment's worst-case event-loop stall, its longest synchronous user-code section or CPU-bound inline activity, not merely above a multiple of the renewal interval**. This is a genuine operator obligation that no construction-time check can enforce for you: the engine cannot measure a stall it is being prevented from running during, so a healthy but momentarily blocked owner can be judged stale and taken over exactly as if it had crashed. Crash takeover itself is bounded by a separate, fixed grace multiplier—see [Rolling deploys, crash takeover, and rollback](#rolling-deploys-crash-takeover-and-rollback) below—but that grace window only dampens clock skew between engines; it does not substitute for sizing the TTL above your own stall risk.

### Storage requirement

`workflow-lease` requires a storage backend with the `conditionalBatch` capability—the same capability `ownership: 'lease'` already requires. `SQLiteStorage`, `LMDBStorage`, and `NeonStorage` all provide it. Construction fails fast with a clear diagnostic if the configured backend lacks it, the same way `ownership: 'lease'` already fails today. A second gate goes further: every engine sharing the store must agree on one fencing mode. The first fencing-mode engine to construct against a fresh store stamps a store-wide `ownership-mode-marker` key recording that mode, and every later fencing-mode engine compares its own configured mode against that marker at construction, failing closed with `OwnershipModeMismatchError` on a mismatch. `ownership: 'none'` engines never touch the marker, so mixing `'none'` with a fencing mode remains an undetected operator responsibility—not a gap this mode introduces, and deliberately out of scope for it.

### Queries and updates against a non-owning engine

`ctx.onQuery()` and `ctx.onUpdate()` handlers only run against the engine that currently holds a workflow's live in-memory context. `query()` and `update()` handle a non-owning engine differently. Under `workflow-lease`, calling `engine.query()` against an engine that has no live local context AND sees another engine's live claim for that workflow now fails closed instead of silently resolving to `undefined`: it throws an error named `WorkflowNotLocallyOwnedError` naming `workflowId`, telling you to retry against the engine that currently holds the claim. A workflow that is simply terminal, purged, or never existed still resolves to plain `undefined`, unchanged. `engine.update()` does not throw the same way: it already routes through a durable, cross-engine-correct coordinated path—a persisted update request delivered and resolved independently of which engine happens to be running the workflow—so a non-owning engine falls through to that path instead of requiring a retry against a specific instance. There is no automatic cross-engine forwarding for queries—if your deployment fronts multiple `workflow-lease` engines with a load balancer, route query traffic by workflow id (or retry against a different instance on `WorkflowNotLocallyOwnedError`) rather than assuming any instance can answer a query for any workflow.

### Ownership signals and alerting

Two operator-facing warning events ship today. Both are `process.emitWarning`-style events exported from the package root, so you subscribe to them the same way you already do for `WeftEngineLeaseLostWarning`:

| Signal                             | Kind                                                                            | What it means                                                                                           | Alert on                                                                                                       |
| ---------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `WeftWorkflowClaimLostWarning`     | `process.emitWarning` event, per occurrence, `workflowId` field                 | This engine's renewal lost its compare-and-swap for one workflow and self-deposed.                      | Any occurrence outside a planned deploy or crash.                                                              |
...
Workflow ids never become metric labels: both `workflowId` occurrences above are warning-event fields, not label dimensions on an exported metric series, matching the repository's bounded-cardinality rule for workflow, operation, worker, and queue identifiers.

ADR 0002 also specifies three bounded-cardinality metrics—`weft_workflow_claim_attempts_total{outcome}`, `weft_workflow_claims_active`, and `weft_workflow_claim_renewal_failures_total`—and an on-demand per-workflow ownership status diagnostic. The engine populates `weft_workflow_claims_active` after every claim-renewal pass and increments `weft_workflow_claim_renewal_failures_total` on every failed renewal. `weft_workflow_claim_attempts_total{outcome}` records `acquired` when an ownerless running workflow is claimed, `takeover` when an expired claim is reclaimed, `lost_race` when a claim attempt loses its compare-and-swap, `deposed` when a renewal discovers another engine now holds the claim, and `backoff_skipped` when the anti-thrash cooldown suppresses a takeover attempt. A sustained rise in `lost_race` or `deposed` means genuine contention—two engines repeatedly reaching for the same workflows—rather than storage trouble, which is what the renewal-failure counter measures. None of the three metrics are wired into the server's Prometheus exporter yet, and the per-workflow diagnostic endpoint does not exist yet either, so until that follow-up lands the two warning events above remain your operator-facing signal for claim loss and stale-wake discards.
