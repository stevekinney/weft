...
The `scheduleToCloseTimeout` is the **cumulative budget across all attempts and the backoff waits between them** (close to Temporal's `scheduleToCloseTimeout`), measured from the first dispatch and _not_ reset between attempts—where `timeout` bounds one attempt, this bounds the whole retry sequence. It is enforced at the **retry-decision point**, not by waiting out the clock: when the next retry's backoff would start the attempt at or after the deadline (or an attempt has already overrun the budget), Weft skips the doomed backoff and fails the activity with an `ActivityScheduleToCloseTimeoutError` (also a `timeout` failure category). It has **no effect unless a retry policy is configured**—a non-retried activity fails on its first error before the budget is consulted—and it applies to top-level `ctx.run` activities only, since an activity inside `ctx.all` / `ctx.race` does not retry.
