...
import { bulkListFilterInputSchema, listFilterFromBulkInput } from './bulk-filter-input.ts';
...
describe('parseBulkListFilterFromBody — validation precedence', () => {
  ...
  it('reports type error before tags error when both are invalid', () => {
    expect(() => parseBulkListFilterFromBody(wrap({ type: 99, tags: 'not-an-array' }))).toThrow(
      'Field "filter.type" must be a string',
    );
  });
  ...
  it('reports limit error before offset error when both are invalid', () => {
    expect(() => parseBulkListFilterFromBody(wrap({ limit: -1, offset: -1 }))).toThrow(
      'Field "filter.limit" must be a non-negative number',
    );
  });
  ...
  it('reports createdAt error before updatedAt error when both are invalid', () => {
    expect(() => parseBulkListFilterFromBody(wrap({ createdAt: 'bad', updatedAt: 'bad' }))).toThrow(
      'Time-range filter must be an object with gte/gt/lte/lt numeric bounds',
    );
  });
