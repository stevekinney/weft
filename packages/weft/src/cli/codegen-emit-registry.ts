 ...
 * produces a single `.d.ts` string that augments the caller-supplied public
 * module name with typed `WorkflowRegistry` entries. The name is a parameter
 * rather than a constant here because the module name the augmentation has to
 * carry is whatever the running weft is *published* as, and that is not the
 * same string in every repository this source lives in: the mirror transform
 * rewrites module specifiers and manifests but deliberately leaves ordinary
 * string constants alone, so a hardcoded name would emit an augmentation of a
 * package that does not exist downstream. `cli/codegen.ts` reads it from
 * weft's own manifest, which is the one place that always says what this
 * package is called. The output is byte-stable across runs with the same input: keys are sorted
 ...
 * the `declare module '<package>' { ... }` block — never inside it.
 * Placing an alias inside the augmentation block would make it a
 * pseudo-public exported type name of that package (autocomplete
...
import { jsonSchemaToTypeScript } from '../index.ts';
import { emitPropertyKey, emitStringLiteral } from '../json-schema/codegen-emit.ts';
import {
...
/**
 * Raised when the caller supplies no package name to augment.
 *
 * The emitter cannot substitute a default. A `declare module 'X'` whose `X` is
 * wrong is not a degraded declaration, it is an augmentation of some other
 * package — or, when nothing answers to that name, an ambient declaration that
 * silently types nothing. Guessing is worse than refusing.
 */
export class CodegenPackageNameError extends Error {}

/**
 * The characters an npm package name may contain, which is also the set that is
 * safe to drop between single quotes without escaping. Validating here rather
 * than escaping is deliberate: a name outside this set is not a quoting problem
 * to be worked around, it is a name npm would reject, and emitting a `.d.ts`
 * around it would turn a bad manifest into a syntax error in somebody's editor.
 */
...
export function emitRegistryDeclaration(
  ...
  if (!packageNamePattern.test(packageName)) {
    throw new CodegenPackageNameError(
      `codegen cannot augment ${JSON.stringify(packageName)}: the generated declaration must name the package its consumer imports, and that has to be a package name npm would accept`,
    );
  }
