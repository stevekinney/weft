...
import { admitCommand, capacityOf } from './mailbox-admission.ts';
import type {
...
import { claimNextCommand } from './mailbox-delivery.ts';
import { decodeApplicationReadyEntry } from './mailbox-index-codec.ts';
import {
  MAILBOX_PRIMITIVE,
  toApplicationCommandReceipt,
  type MailboxRuntime,
} from './mailbox-internals.ts';
import { runMailboxMaintenance } from './mailbox-maintenance.ts';
import {
  acknowledgeClaim,
  readCleanupState,
  rejectClaim,
  renewClaim,
  requestCancellation,
} from './mailbox-settlement.ts';
import { createMailboxKeys, loadCommand, loadMailboxHeader } from './mailbox-storage.ts';
import {
  ...
  validateFailure,
} from './mailbox-validation.ts';
import { waitForAvailableWork, waitForCleanup } from './mailbox-waits.ts';
...
export class Mailbox {
   ...
   */
  async list(options?: MailboxListOptions): Promise<ApplicationCommandReceipt[]> {
    this.#assertLive();
  ...
  async capacity(): Promise<MailboxCapacity> {
    this.#assertLive();
   ...
   */
  async waitForAvailable(options?: MailboxWaitOptions): Promise<boolean> {
    this.#assertLive();
   ...
   * one call.
   */
  async runMaintenance(now = this.#runtime.now()): Promise<MailboxMaintenanceReport> {
    this.#assertLive();
