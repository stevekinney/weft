...
import { isWaitingState, loadCommand, loadDeliveryHead } from './mailbox-storage.ts';
import { recoverExpiredCommand } from './mailbox-transitions-recovery.ts';
import { claimWaitingCommand } from './mailbox-transitions.ts';
import type { ApplicationCommandRecord } from './mailbox-types.ts';
import { requireClockInstant, requireGeneratedIdentifier } from './mailbox-validation.ts';
...
): Promise<MailboxClaimResult | { readonly lost: boolean }> {
  const now = runtime.now();
...
): Promise<MailboxClaimResult | null> {
