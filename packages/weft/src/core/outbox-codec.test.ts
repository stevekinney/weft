...
describe('index codecs', () => {
  ...
  it('round-trips the header and rejects inconsistent counters', () => {
    ...
    expect(decodeOutboxRecord(encodeOutboxRecord(header), HEADER)).toEqual(header);
    for (const broken of [
      { ...header, openCount: 4 },
      ...
      expect(() => decodeOutboxRecord(encode(broken), HEADER)).toThrow(PersistedDataCorruptError);
    }
    expect(() => decodeOutboxRecord(new Uint8Array([9]), HEADER)).toThrow(
      PersistedDataCorruptError,
    );
  });
