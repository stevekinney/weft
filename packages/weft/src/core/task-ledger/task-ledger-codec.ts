...
export function isBoundedReason(value: unknown): value is string {
  return (
...
export function isOptionalBoundedReason(value: unknown): boolean {
...
/**
 * Whether `value` is a valid {@link WorkerExecutionRequirementInput} — exported
 * so `task-attempt-codec.ts` can validate a `TaskAttemptRecord`'s carried
 * routing requirement (COR-205, acceptance criterion 3) with the exact same
 * bounds this module enforces on a `RemoteTaskRecord`'s own field, rather
 * than a second, potentially drifting copy of the same check.
 */
export function isValidExecutionRequirement(
  value: unknown,
): value is WorkerExecutionRequirementInput {
  if (value === undefined) return true;
  if (!isRecord(value)) return false;
  return (
...
/**
 * Whether `value` is a valid {@link WorkerExecutionIdentity} — exported for
 * the same reason as {@link isValidExecutionRequirement}: `task-attempt-codec.ts`
 * validates a `TaskAttemptRecord`'s carried actual-execution identity
 * (COR-205, acceptance criteria 1 and 9) against this exact bound rather than
 * re-deriving it.
 */
export function isValidExecutionIdentity(value: unknown): value is WorkerExecutionIdentity {
  if (!isRecord(value)) return false;
  return (
...
function isValidLeaseHolderShape(value: Record<string, unknown>): boolean {
  return (
    ...
    isOptionalFiniteNumber(value['attemptDeadline']) &&
...
export function isRemoteTaskCancelling(value: unknown): value is RemoteTaskCancelling {
  ...
  return (
    ...
    isFiniteNumber(value['cancellationDeadline'])
  );
}
...
/**
 * Narrow an unvalidated ledger record to a task that reached a terminal state
 * on its own — it completed or it failed, and the attempt that owned it said
 * so.
 *
 * Records read back from storage arrive as `unknown` because the durable
 * ledger is hostile input: another process wrote it, possibly an older build.
 * This validates the whole shape, not just the discriminant, so a truncated or
 * hand-edited record is rejected rather than trusted.
 *
 * Pair it with {@link isRemoteTaskTerminalCancelled}: both mean "finished",
 * but only this one means the work itself produced an outcome.
 *
 * @example
 * ```ts
 * import { isRemoteTaskTerminalResolved } from '@lostgradient/weft';
 *
 * declare const record: unknown;
 *
 * if (isRemoteTaskTerminalResolved(record)) {
 *   console.log(record.status, 'at', record.terminalAt);
 * }
 * ```
 */
...
/**
 * Narrow an unvalidated ledger record to a task that was cancelled rather than
 * allowed to finish.
 *
 * The counterpart to {@link isRemoteTaskTerminalResolved}: both describe a
 * task that will not run again, but this one carries a `cancellationReason`
 * and no outcome. Watch for `uncertain` — when set, cancellation was requested
 * but the worker never confirmed, so the activity may in fact have run. Any
 * reconciliation or billing pass has to treat that as "unknown", not "did not
 * happen".
 *
 * @example
 * ```ts
 * import { isRemoteTaskTerminalCancelled } from '@lostgradient/weft';
 *
 * declare const record: unknown;
 *
 * if (isRemoteTaskTerminalCancelled(record)) {
 *   console.log(record.cancellationReason);
 *   if (record.uncertain === true) console.log('may have run anyway — verify');
 * }
 * ```
 */
