...
import { requireDerivedInstant } from './outbox-guards.ts';
import type {
 ...
 */
export type OutboxTransitionRejection =
  | 'stale-attempt'
  | 'not-leased'
  | 'not-waiting'
  | 'not-due'
...
export type OutboxTransition<TNext> =
  ...
  | { readonly ok: false; readonly reason: OutboxTransitionRejection };

export function rejectedTransition<TNext>(
  reason: OutboxTransitionRejection,
): OutboxTransition<TNext> {
  return { ok: false, reason };
}

export function succeededTransition<TNext>(next: TNext): OutboxTransition<TNext> {
  return { ok: true, next };
}
