...
import { collectKeys } from '../testing/storage-backends.test-support.ts';
...
describe('Mailbox sustained contention', () => {
  ...
  it('carries the operation and command as structured fields', () => {
    const withCommand = new MailboxContentionError('claim', 'c-1');
    expect(withCommand.code).toBe('MailboxContentionError');
    ...
    const mailboxWide = new MailboxContentionError('admit', null);
    expect(mailboxWide.commandId).toBeNull();
    expect(mailboxWide.message).not.toContain('for command');
  });
});

describe('Mailbox event-sink failure classification', () => {
  it('propagates a sink failure when the caller conditions still hold', async () => {
    ...
    const persisted = await collectKeys(
      storage,
      KEYS.applicationCommandPrefix('bureau', 'agent-7'),
    );
...
describe('Mailbox digest failures', () => {
...
describe('Mailbox persisted causation', () => {
...
describe('Mailbox reference payload without a declared size', () => {
...
describe('Mailbox retention edge cases', () => {
  it('retires a terminal index entry whose command record is already gone', async () => {
    ...
    const remaining = await collectKeys(
      storage,
