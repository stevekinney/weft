 ...
 * strict FIFO.
 *
 * @module core/application-outbox-delivery
 */
...
import { requireClockInstant, requireGeneratedIdentifier } from './application-outbox-guards.ts';
import {
...
import { loadDelivery, loadDueHead, type DueEntry } from './application-outbox-storage.ts';
import { claimWaitingDelivery } from './application-outbox-transitions.ts';
import {
...
export async function claimNextDelivery(
  ...
  throw new ApplicationOutboxContentionError('claim', null);
}

async function attemptClaim(
  runtime: OutboxRuntime,
  signal: AbortSignal | undefined,
): Promise<ApplicationOutboxClaimResult | { readonly lost: boolean }> {
...
): Promise<ApplicationOutboxClaimResult | null> {
