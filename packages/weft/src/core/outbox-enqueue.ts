...
import { ApplicationDeliveryValidationError, requireGeneratedIdentifier } from './outbox-guards.ts';
import {
  encodeApplicationDeliveryEntry,
  encodeApplicationDeliveryIdempotencyRecord,
} from './outbox-index-codec.ts';
import {
...
import { createEnqueuedDeliveryRecord } from './outbox-transitions.ts';
import { OUTBOX_RECORD_VERSION } from './outbox-types.ts';
import { validateDeliveryInput } from './outbox-validation.ts';
...
export function capacityOf(runtime: OutboxRuntime, open: number, enqueued: number): OutboxCapacity {
