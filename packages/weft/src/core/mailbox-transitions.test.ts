...
} from './mailbox-transition-helpers.ts';
...
describe('mailbox record codec', () => {
  ...
  it.each([
    ...
    for (const decode of [decodeMailboxRecord, decodeApplicationCommandIdempotencyRecord]) {
  ...
  it('names the offending key in the diagnostic', () => {
    try {
      decodeMailboxRecord(new Uint8Array([0xc1]), 'appmbx:v1:bureau:agent-7');
