...
// StandardSchemaValidationError is public via the `@lostgradient/weft` subpath,
...
const cases: Record<WeftErrorCode, () => WeftError> = {
  ...
  MailboxContentionError: () => new MailboxContentionError('admit', null),
  ApplicationDeliveryValidationError: () =>
    new ApplicationDeliveryValidationError('destinationRef must be a non-empty string.'),
  OutboxContentionError: () => new OutboxContentionError('enqueue', null),
