...
describe('Mailbox backlog policy', () => {
...
describe('Mailbox input validation', () => {
  it.each([
    ['caller', { caller: '' }],
    ['target', { target: '' }],
    ['kind', { kind: '' }],
...
describe('Mailbox hostile persisted records', () => {
  it.each([
...
describe('Mailbox disposal', () => {
  ...
  it('supports `using` disposal', async () => {
    ...
    const reopened = new Mailbox({ storage, namespace: 'n', resourceId: 'r' });
