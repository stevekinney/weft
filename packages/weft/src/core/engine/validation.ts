...
/**
 * Drop a decoded `revision` that is not a non-empty, bounded string —
 * mirrors {@link sanitizeDecodedParentLineage}'s tampering-defense pattern.
 * `revision` is engine-derived (never user-supplied at the API surface), so
 * an invalid value here indicates storage corruption or tampering rather
 * than ordinary bad input; absence after this normalization is the same
 * "legacy, pre-this-field record" signal a genuinely missing `revision`
 * carries, so recovery classification treats both alike.
 */
function sanitizeDecodedRevision(state: WorkflowState): void {
