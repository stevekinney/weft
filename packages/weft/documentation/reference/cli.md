...
| `--console`   |       | `false`     | Mount `@lostgradient/weft-ui` and its static assets       |
...
The generated file augments the public `weft` module with `WorkflowRegistry` entries — typed workflow names that make `engine.start('name', ...)` infer the right input shape and also carry the active `revision` and `workflowVersion` as compile-time string-literal fields for introspection. Neither field is required by `engine.start`/`WeftClient.start`/`.schedule()` — an ordinary start still needs no caller-supplied revision. Activity names are no longer typed via a global module augmentation; they live on the workflow builder's `.activities({...})` step instead. Output is byte-stable: running the command again with the same snapshot reports that the file is up to date and does not rewrite it. When two or more workflows share an identical, non-trivial input or output schema, the emitter hoists it into one shared `type` alias declared above the `WorkflowRegistry` augmentation, so identical schemas aren't repeated inline at every call site.

The source registry snapshot (`registryVersion: 2` — see [`api-server.md`](./api-server.md#registry-snapshot)) carries `workflows` as an array of `WorkflowRevisionManifest` entries and `activeRevisions` as a `name -> revision` pointer map; `weft codegen` resolves each name's currently active manifest via that pointer and reads `inputSchema`/`outputSchema`, `revision`, and `workflowVersion` off it for declaration generation. Each manifest's `contract` also carries its workflow's statically declared signal, update, and query names with their available input/output JSON Schemas — `weft codegen` does not emit these today, but they, and the finalizer contract, are available to any tool that reads the snapshot directly. Activity entries (unchanged from `registryVersion: 1`) include their registered retry policy and timeout alongside queue and schema metadata. `weft codegen` currently uses workflow input/output schemas plus revision metadata for declaration generation; activity schemas and the additional operator metadata remain available directly from `GET /v1/registry`.
...
`revision` and `workflowVersion` are string-literal types taken directly from the workflow's active `WorkflowRevisionManifest` — see the [Revision Identity](../guides/workflow-versioning.md#revision-identity) guide. Neither is required by `engine.start`, `WeftClient.start`/`.schedule()`, or `.result()`; they exist purely for a consumer's own compile-time introspection (asserting an expected pinned revision, for example). When two or more workflows share the exact same non-trivial input or output schema, the emitter hoists that schema into a single unexported `type` alias declared above `declare module '@lostgradient/weft'` and references it from every entry that uses it, rather than repeating the same inline type at each call site.
