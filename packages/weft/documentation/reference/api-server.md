...
| `weft.workflows.revisions.preload`  | `POST /v1/registry/workflows/:name/preload`            | `workflows:admin` | Body: `{ "revision" }`. Loads, validates, and installs a `registerSource()`-registered revision (WFT-15/16). Non-destructive. `404` when no source was ever registered for this exact key; `409` when the load fails or the loaded module fails validation. |
...
If a fleet cursor predates the retention floor, replay begins with one `fleet:gap` envelope. The envelope carries the requested cursor in `payload.requestedCursor` and the first available sequence in `payload.firstRetainedSequence`; subsequent frames are retained events in cursor order. JSON-RPC `weft.events.subscribe` uses the same envelope and ordering.

JSON-RPC over HTTP remains request/response only. JSON-RPC clients can subscribe to per-workflow events with `weft.workflows.subscribe` or to one fleet-wide event feed with `weft.events.subscribe` over WebSocket or stdio; both deliver notifications as `weft.events.deliver`. Fleet-wide subscriptions expose workflow-facing operational events and worker connection lifecycle events, accept optional `workflowId` and `kind` filters, reject replay windows above 1,000 matching retained events, and allocate one ordered cursor space across server processes that share the same compare-and-swap-capable durable store. The finite token replay SSE route, `/api/v1/workflows/:id/sse`, remains a separate historical token stream and does not emit live `ping` keepalives.
