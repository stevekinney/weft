...
| `ApplicationCommandValidationError`         | `ApplicationCommandValidationError`         | An `ApplicationMailbox` was constructed on storage lacking conditional-batch support, snapshot scan consistency, or linearizable read-after-write; used after disposal; or offered a command or command id with a missing, malformed, oversized, un-hashable, or out-of-range field. | Caller mistake; fix the option or command before retrying. A full backlog and an idempotency conflict are returned as results, not thrown                                                                                                                     |
| `ApplicationMailboxContentionError`         | `ApplicationMailboxContentionError`         | An `ApplicationMailbox` transition lost its compare-and-swap precondition on every one of its bounded attempts, so durable contention on that one mailbox is sustained.                                                                                                              | Transient; back off and retry, or shard the resource. Carries `operation` and `commandId` for diagnosis                                                                                                                                                       |
| `WaitBudgetElapsedError`                    | `WaitBudgetElapsedError`                    | `ApplicationMailbox.awaitCleanup()` with a positive `timeoutMs` ran out of budget while its first cleanup-state read was still in flight, so there is no observation to report as `pending`.                                                                                         | Transient; nothing durable changed. Wait again, or read `cleanupState()` directly                                                                                                                                                                             |
...
| `WorkflowSourceValidationError`             | `WorkflowSourceValidationError`             | `engine.resolveWorkflowSource()` loaded a module that failed validation — a missing or ambiguous export, a non-builder-produced definition, an oversized contract, or a `name`/`revision`/`workflowVersion`/`contractHash` mismatch against the descriptor's expectations.           | Catchable rejection; carries `workflowName`, `revision`, and the full `reasons: WorkflowSourceRejectionReason[]` array — fix the source module or the descriptor and retry                                                                                    |
