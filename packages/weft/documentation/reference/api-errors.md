...
| `ApplicationCommandValidationError`         | `ApplicationCommandValidationError`         | An `ApplicationMailbox` was constructed on storage without conditional-batch support, used after disposal, or offered a command with a missing, oversized, undigestable, or out-of-range field. | Caller mistake; fix the option or command before retrying. A full backlog and an idempotency conflict are returned as results, not thrown                                                                                                                     |
