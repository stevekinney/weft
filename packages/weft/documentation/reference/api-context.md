...
> `ctx.log` is a host-owned, process-local observability surface. Weft does not persist records or expose an engine, client, REST, or JSON-RPC log-history API, so a remote console must not expect to fetch prior logs from Weft. Install [`EngineOptions.onLog`](configuration.md#engineoptions) to route every non-replayed inline and worker-mode record into a logging system that provides retention and queries (pino / winston / OpenTelemetry, etc.). When a sink is set, the console is no longer also called, so there is no duplicate noise. The sink receives a `WorkflowLogRecord` (both `WorkflowLogRecord` and `WorkflowLogLevel` are exported for typing it) and is never invoked for records suppressed during replay. If the sink throws, Weft catches that logging failure, sends the record to the matching console method, and lets the workflow continue; `ctx.speculate()` branches use the same sink and fallback behavior. Inline timestamps come from the engine clock; worker-mode timestamps come from the worker process wall clock.
