...
Fleet replay is retention-aware. When a requested cursor is older than the retained history, both fleet transports deliver one `fleet:gap` envelope before the first retained event. Its `payload.requestedCursor` records the cursor supplied by the consumer and `payload.firstRetainedSequence` identifies the first sequence that can be replayed. The gap is an explicit compaction signal, not an application event.

SSE data frames use `id: <cursor>`, `event: <event kind>`, and `data: <JSON event envelope>`. Reconnect with `Last-Event-ID: <cursor>`; that header takes precedence over `fromCursor=<cursor>`. Streams emit `event: ping` with JSON metadata and no `id`, so keepalives prove the connection is alive without advancing the replay cursor. The initial replay handoff is also a `ping` frame with `replayComplete: true`; clients can treat that cursorless frame as the point where replay has drained and live delivery is active. Stream errors after headers are sent are reported as sanitized `event: error` frames before the server closes the connection.
