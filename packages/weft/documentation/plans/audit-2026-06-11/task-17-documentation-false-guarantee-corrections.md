...
- [ ] migration.md surfaces every breaking change recorded in CHANGELOG.md as actionable migration steps — at minimum the 0.3.0 multi-tenancy removal and the 0.1.0 agent-surface removal. (The root `BREAKING-CHANGES.md` is a stub that points at migration.md — leave that pointer intact; the stability policy lives separately in `documentation/contributing/breaking-changes.md` and is not part of this task.)
