...
The fence is also not absolute. Its compare-and-swap tests a value, so it cannot tell "this id was never used" from "a run existed here and was purged": if a racing winner completes and is purged (or swept by retention) before the losing start commits, the id looks free again and both runs execute. Closing that needs durable per-id generation state a purge cannot restore. The window is narrow, and the outcome is a duplicate run rather than a hang, but do not read the fence as a guarantee of uniqueness under a topology that does not support one.
