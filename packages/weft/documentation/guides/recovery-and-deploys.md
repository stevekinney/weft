...
A start with an explicit `id` is the one place where that misconfiguration is caught for you rather than silently corrupting the store. The duplicate-id check and the create commit are atomic: the create batch carries a compare-and-swap on the workflow record as the duplicate-id check observed it, so if a second engine commits a run for the same id in between, the losing start rejects with `WorkflowAlreadyExistsError`—the same error a duplicate id raises within a single engine. Previously both writes committed blind, the second overwrote the first, and the loser's `handle.result()` never settled: its run terminated on the other engine, which is the only place that engine's completion waiter could have been resolved.

That error is not by itself evidence of a two-engine store. A correctly configured single engine raises the same `WorkflowAlreadyExistsError` for an ordinary duplicate id—a sequential reuse, or a same-tick collision caught by the in-process reservation—and that is the common case. Read it as a possible indicator worth checking against your deployment topology, not as proof of one, and confirm a cross-engine race independently before acting on it.

The fence's compare-and-swap also carries a second condition, on a durable per-id generation counter that a purge bumps but never resets (`wf-gen:<id>`). Earlier releases had only the value-only condition above, which by itself cannot tell "this id was never used" from "a run existed here and was purged": if a racing winner completed and was purged (or swept by retention) before the losing start committed, the id looked free again and both runs executed. The generation condition closes that: a purge always changes `wf-gen:<id>`, even though it leaves `wf:<id>` looking identically absent, so the losing start's compare-and-swap now fails on the generation condition where the value-only one could not detect anything wrong.

Where it does come from two engines, the guard covers only the start itself. Recovery, timers, and signal delivery remain uncoordinated, and a run this check lets through can still be resumed twice, so it is a diagnostic rather than a substitute for `ownership: 'workflow-lease'`. Starts with a generated id are unaffected: a v4 UUID cannot collide, so those keep the unconditioned single-write path.
...
**Activities, finalizers, and constraints now recover correctly per-revision too, not just the generator/checkpoint replay (WFT-19).** The activity-registry caveat from before this release is closed: a recovered instance's `ctx.run('name')` calls now resolve through its OWN exact `(type, revision)` pin, not a shared, type-only registry a sibling revision's concurrent resolve could clobber. This closes a real gap in `recoverAll()` too: resume/recovery never populated the per-instance identity cache activity dispatch depends on before this release, so a builder workflow's string-named activity call could fail to resolve at all on its very first turn after a fresh-process recovery — see [Activity, finalizer, constraint, and retention routing follow the pin too](workflow-versioning.md#activity-finalizer-constraint-and-retention-routing-follow-the-pin-too-wft-19) in the versioning guide for the full list of routing paths this closes.
