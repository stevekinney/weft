...
Treat that error as the signal it is. It means two engines are sharing a store under `'none'`, and this guard covers only the start itself—recovery, timers, and signal delivery remain uncoordinated, and a run this check lets through can still be resumed twice. It is a diagnostic, not a substitute for `ownership: 'workflow-lease'`. Starts with a generated id are unaffected: a v4 UUID cannot collide, so those keep the unconditioned single-write path.
