...
> [!NOTE]
> The generator workflow API is the primary engine surface. [`ctx.step()`](../reference/api-context.md#step-based-workflows) and [`compileStepWorkflow(...)`](../reference/api-context.md#step-based-workflows) are experimental sugar before 1.0; use them when they improve readability, but expect the generator API to define the stable semantics first. See the [canonical stability-tier inventory](../contributing/breaking-changes.md#stability-tiers).
...
> You can register a `finalizer` on a worker-mode engine — registration no longer throws — and the teardown always runs on the engine host, even when `activityExecution` is configured separately. But durable teardown only fires when the workflow staged state with `ctx.setFinalizerState`, and that call works only under inline execution. A handler annotated `(ctx: WorkflowContext)` type-checks the call in any generator (the static type declares it), but at runtime in worker mode the generator's `ctx` is a reduced worker-side context that does not carry `setFinalizerState` at all, so the call fails with a `TypeError` — and there is no host-side API to stage the state for you. So a worker-mode workflow that needs durable finalizer teardown must run in inline mode instead.

`ctx.onCancel` is inline-only at runtime for the same reason `ctx.setFinalizerState` is. Both type-check on a `(ctx: WorkflowContext)` handler because the static type declares them, but the worker-side context the generator actually receives at runtime carries neither, so calling either in worker mode fails with a `TypeError`. (Inside a `ctx.speculate()` branch — which reuses the inline `ctx` — the methods _are_ present, but the engine leaves their callbacks unset, so they throw a descriptive guard error instead.) A worker-mode workflow that needs best-effort in-process cleanup should do it in the workflow body instead (a `ctx.run` destroy step inside a `try/finally`).
