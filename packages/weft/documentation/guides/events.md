...
`HttpClient` tails use `eventTransport: 'auto'` by default: they prefer the `/v1/workflows/:id/watch` WebSocket channel, then fall back to fetch-based SSE at `/v1/workflows/:id/events/sse` when the runtime cannot construct an authenticated WebSocket. WebSocket tails catch up from `getEvents()` on connect and reconnect; raw watch sockets accept `?resumeFrom=<sequence>` and include `sequence` / `cursor` on each event frame. SSE tails reconnect with `Last-Event-ID`, ignore cursorless `ping` keepalives, wait for the `ping` whose JSON metadata includes `replayComplete: true`, and resume from the server event-feed cursor. JSON-RPC clients can use `weft.workflows.subscribe` for one workflow or `weft.events.subscribe` for the fleet-wide event feed. Fleet subscriptions can filter by workflow id and event kind; they reject replay windows above 1,000 matching retained events, and server processes that share the same compare-and-swap-capable durable store allocate one ordered fleet cursor space. `LocalClient` tails bridge the engine event stream and perform the same history catch-up. A tail is single-consumer: open a fresh `client.tail(id)` or `handle.tail()` for each independent reader.

When a fleet cursor is below the retention floor, both SSE and JSON-RPC emit one `fleet:gap` envelope containing `requestedCursor` and `firstRetainedSequence` before continuing with retained events. Consumers should treat this boundary as a compaction notice and persist the gap envelope's own cursor until they consume a retained event; persisting `firstRetainedSequence` early would skip an event later written at that sequence because reconnect cursors are exclusive.
