...
`Outbox` is the primitive for that case. It makes the _intent_ durable before any attempt begins, records what the transport reported as evidence rather than as truth, and settles each delivery only through the durable disposition the outbox commits on that evidence. It is the outbound sibling of the [application mailbox](mailbox.md), built on the same fencing, cancellation, and recovery machinery.
...
`retry({ deliveryId })` returns an `unknown-outcome`, `dead-lettered`, or `rejected` delivery to the due index with at least one more attempt to spend: a budget the delivery never used up (a permanent rejection on its first attempt of five, say) is kept, so the reopened delivery may retry on its own up to its `maxAttempts`, while a spent budget is raised to `attempt + 1`, so the delivery can be claimed exactly once more. Reopening is an admission, so it respects `maxBacklog` and reports `rejected` with the current capacity when the backlog is full, and the record's provenance (`attempt`, `retryCount`) is preserved. `deadLetter({ deliveryId, reason })` closes a parked delivery for good. Both are ordinary compare-and-swap transitions, and both emit a fleet event; a retry is labelled `outbox:delivery-retried` so a feed consumer can tell it from a first enqueue.

## Drain and Shutdown

`drain({ timeoutMs })` delivers everything that is due, runs a maintenance pass before its first round and whenever a round finds nothing due (so lapsed leases are recovered without rescanning the outbox before every send), and waits — bounded, and never longer than `pollIntervalMs` at a stretch, so a delivery another process enqueues meanwhile is seen promptly — for held deliveries to come due. The budget is one stop signal for the whole drain: it ends the sleeps, the maintenance passes, an in-flight send, and the wait for a settlement alike, so a drain asked to stop at a deadline stops there — a claim, settlement, or maintenance pass still waiting on storage when it fires finishes on its own (sending nothing once it sees the signal) and is not counted by that drain. It reports counts only:
...
Every mutation retries a bounded number of times against freshly read durable state, then throws `OutboxContentionError`. The header is a per-outbox hot key — enqueue and every terminal transition touch it, which is what keeps `capacity()` exact — so a busy owner does see contention. Surfacing it beats spinning.
