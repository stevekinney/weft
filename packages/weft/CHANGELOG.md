...
- `ForkOptions.revision?: string` (WFT-21) — an explicit, validated opt-in to fork against a DIFFERENT installed revision than the source run's own pin, for diagnostic use ("does this input fail on v1 or only on v2?"). Omitted (the default): the fork resolves and persists the SOURCE run's own revision, so for a **dynamic-source** workflow, differences between a run and its fork come only from input or history choices, never a silently different revision of the code. This guarantee does NOT extend to an **eager-registered** workflow type: `resolveExecutableRegistrationForRevision()` deliberately runs whatever this process currently has eagerly registered regardless of the persisted pin, the same "eager is always ready" exception the Known limitation entry below already documents for recovery (WFT-159) — a default fork of an eager type can execute different code than its source run pinned, even though the persisted `revision` field itself is unaffected. Validated BEFORE any checkpoint is read: for an eager-registered type, `revision` must exactly equal the revision this process loaded; for a dynamic-source type, it must name one of this process's currently registered, resolvable candidates. Either mismatch throws `WorkflowRevisionUnavailableError` (`reason: 'not-registered'`) with no partial write. A semver-incompatible target `version` still throws `VersionMismatchError` via the existing `derivePreparedExecutionState()` compatibility gate — an explicit revision never bypasses ordinary compatibility checking. `weft.workflows.fork`'s operation input gains a matching optional `revision` field (REST body field and JSON-RPC param, and now correctly serialized by the HTTP client's `fork()` request); a `WorkflowRevisionUnavailableError` from `engine.fork()` maps to a `Conflict` (409) fault with `data.reason` over JSON-RPC (REST exposes it in the error message text instead, per the existing "REST never discloses `Conflict.data.reason`" design) via a new shared `mapRevisionUnavailableToFault(error)` helper (`server/operations/revision-unavailable-fault.ts`), also reused by `schedule-faults.ts`. The fork's own commit is fenced against a concurrent `removeWorkflowRevision()` targeting its persisted revision through two layers. The commit durably fences on the target revision's catalog entry (`buildForkCatalogEntryCondition`) under every ownership mode, including the default `ownership: 'none'` — this durable fence is unconditional, not lease-only. In addition, `fork()` reserves an in-memory `inFlightStartsByRevision` slot for its target revision as early as validation completes, released unconditionally once the commit settles — including the legacy case (a source run predating revision pinning, on a dynamic-source type with exactly one registered candidate), where the resolver's own resolved revision is reserved via `reserveLegacyForkTargetRevision()` synchronously, before any loader await. This in-memory reservation is process-local, so under a multi-process deployment it protects only same-process races; the durable catalog-entry fence is what closes the cross-process race regardless of ownership mode.
- `WorkflowReplay.revision?: string` (WFT-21) — the revision of the code that produced `engine.replayTo(workflowId, step)`'s reconstructed state, sourced from the run's own persisted `WorkflowState.revision`. Omitted when the workflow record has since been purged, predates revision pinning (a legacy record with no persisted `revision`), or when a `workflowExecutionToken` mismatch between the loaded checkpoint and the current `WorkflowState` (re-read after the later checkpoint/event-log reads) indicates the run id was displaced by a concurrent `start(..., { onTerminalConflict: 'start-new' })` replacement — reporting `undefined` rather than misattributing another generation's revision. REST and JSON-RPC both pass it through unchanged.
- `WorkflowRevisionReferenceCounts.retainedRecoveryRecords` is no longer a permanent `0` stub (WFT-21) — closing the final gap `WorkflowRevisionReferenceCounts` (WFT-12) left in place. It now sums two real, storage-scan-backed durable-reference components — a terminal (`completed`/`failed`/`cancelled`/`timed-out`) `WorkflowState` still present (not yet purged) and pinned to exactly this revision, plus a `TeardownDeadLetterRecord` (a permanently failed finalizer) pinned to this revision — and neither scan is bounded: `countWorkflowStateRevisionsByStatus()` walks the entire `wf:`-prefix namespace and `countTeardownDeadLettersForRevision()` walks both dead-letter namespaces, each O(n) in the total number of records rather than scoped to `(name, revision)`; `removeWorkflowRevision()` also calls `countWorkflowRevisionReferences()` (`catalog-removal.ts`) — and therefore both scans — TWICE per removal attempt: its own pre-check, and `finalizeRevisionRemoval()`'s post-check — worth knowing before wiring `removeWorkflowRevision()` into an automated cleanup path on a large durable store. `removeWorkflowRevision()` now correctly refuses a revision either references. **Behavior change, not a silent bugfix**: an operator who could previously remove a revision with an old terminal-but-unpurged run or a dead letter (because the field was always `0`) can no longer do so without releasing that reference first. The terminal-`WorkflowState` component is released by an ordinary workflow purge or retention sweep. The dead-letter component is deliberately **never** auto-released — a dead letter is excluded from the purge delete-set as permanent leak evidence, so a revision that ever dead-lettered stays non-removable indefinitely until a future acknowledge/clear API exists (not built this batch — see the [Reference Accounting and Removal](documentation/guides/workflow-versioning.md#reference-accounting-and-removal) guide). Also fixes a pre-existing gap in `catalog-tombstone-recovery.ts`'s boot-time orphan sweep, which previously checked only non-terminal runs: it now checks all three durable reference kinds, matching `removeWorkflowRevision()`'s own pre/post checks.
- New durable namespace `wf-teardown-deadletter-history:<workflowId>:<workflowExecutionToken>` (WFT-21) recording one dead-letter record per generation, keyed by the generation's own token. Fixes a real bug: `deadLetterTeardown()` previously wrote each dead letter to a single `KEYS.teardownDeadLetter(workflowId)` slot keyed by workflow id alone, so a workflow id reused across generations (purge, or `onTerminalConflict: 'start-new'`) had a later generation's dead letter silently overwrite an earlier generation's record and its reference count. `KEYS.teardownDeadLetter(workflowId)` itself is unchanged and keeps serving "the latest dead letter for this workflow id." A dead-lettering run with no `workflowExecutionToken` (a legacy, pre-token run) uses a fixed sentinel history segment (`LEGACY_DEAD_LETTER_HISTORY_TOKEN`) rather than its own token — a bounded, deliberately-accepted edge case: a SECOND token-less run reusing the same workflow id and also dead-lettering still collides on that shared sentinel segment, silently losing the earlier legacy record's reference (`retained-recovery-record-count.test.ts` covers this bounded case explicitly). Added to `WEFT_RESERVED_KEY_PREFIXES` and `storage/interface.test.ts`'s representative-key coverage assertion.
- `Checkpoint` gains an additive optional `workflowExecutionToken?: string` field (WFT-21), stamped once at a run's initial checkpoint (a fresh `start()` or `fork()`) and carried forward unchanged by every later `advanceCheckpoint()` call for that execution, backfilled from `WorkflowState` on a pre-upgrade checkpoint lineage's recovered live checkpoint. Used to correlate a checkpoint with its owning `WorkflowState` generation for `replayTo()`'s revision attribution, and — for worker-mode workflows — to fence a worker-returned checkpoint's `persistWorkerCheckpoint()` commit: a worker's claimed token is compared against the host's own current in-memory record of the generation, and the commit is rejected outright on a mismatch rather than silently reattaching a possibly-stale token.
- New permanent key prefix `catalog-removal-generation:<name>:<revision>` (WFT-21) — a monotonic counter, retained forever once written (never deleted by finalize, restore, release, purge, or retention), bumped atomically by `removeCatalogEntry()` alongside its delete+tombstone write. `runSharedSourceLoad()` (`core/engine/source-resolution.ts`) captures its bytes before invoking a dynamic-source loader and threads them through to `catalog.install()` as an additional CAS condition, so a full remove-and-finalize cycle for the same `(name, revision)` landing while that load is still in flight — even after its own tombstone has already resolved — now fails the fenced install closed instead of resurrecting the just-removed revision. Added to `WEFT_RESERVED_KEY_PREFIXES` and `interface.test.ts`'s representative-key coverage assertion.
- New root-exported `ForkSourceReplacedError` (`core/engine/fork-source-replaced-error.ts`), mapped to a `Conflict` fault by `resolveForkAccess()`. `fork()` correlates its source run's `WorkflowState.workflowExecutionToken` against the loaded source checkpoint's own token immediately after hydration, and revalidates it again by re-reading `WorkflowState` immediately before its own commit, rejecting the fork outright on either mismatch rather than risking a fork that mixes an earlier generation's type/input with a later generation's checkpoint when the source run is concurrently replaced (`onTerminalConflict: 'start-new'`). The caller re-issues `fork()` against the replacement.
- Checkpoint commits for a freshly-started workflow (`start()`) and for a newly-forked run (`fork()`) are now `conditionalBatch`-fenced against their own last committed bytes from their very FIRST commit onward, on any storage backend supporting `conditionalBatch` — previously only a resumed/recovered run's checkpoint chain was fenced from its own re-launch. Closes a window where a stale commit for a displaced generation could overwrite a concurrent `start-new` replacement's checkpoint and history.

### Changed

- `WorkflowCatalog.install()`, `resolveEntry()`, `hasInstalled()`, `activateCandidate()`, and `activateRegistered()` (WFT-21) now revalidate a cache hit against durable storage, or fence their own commit on the target entry's current bytes, instead of trusting a stale in-process cache or an unfenced active-pointer write. Closes several races where a peer's concurrent `removeWorkflowRevision()` could otherwise leave a cached read, or the catalog's active pointer, naming a revision that no longer durably exists.
- `resolveOrphanedCatalogTombstones()` (the boot-time orphaned-catalog-tombstone sweep run by `ensureWorkflowCatalogReady()`) now isolates each tombstone's DECODE-or-reference-count resolution failure independently instead of letting it propagate out of the whole sweep and block every start/resume/recovery on the engine. An isolated failure dispatches a new `CleanupWarningEvent`, and leaves that one tombstone's revision conservatively unresolved (restored on an ambiguous reference-count failure, left untouched on a corrupt tombstone) rather than guessing. A malformed tombstone KEY itself (not its bytes) is a narrower, deliberately unisolated exception: `splitCatalogTombstoneKey()` still throws before the per-tombstone isolation is entered, and that still fails the WHOLE sweep — and therefore `ensureWorkflowCatalogReady()`, blocking every start/resume/fork/recovery — closed, since a malformed key can only mean storage corruption or a foreign write into this namespace with no safe per-record default.
- Several revision-related engine faults that previously surfaced as an opaque `EngineFailure` (REST 500 / undeclared JSON-RPC failure) now map to their documented `Conflict` (409) fault: an explicit-revision fork's `VersionMismatchError`, and a `WorkflowRevisionUnavailableError` surfaced by `weft.workflows.revisions.install()`/`preload()` racing a concurrent removal, or by a dynamic-source load racing a concurrent removal (previously re-wrapped into a generic `DynamicWorkflowSourceUnavailableError` by `loadAndInstallSourceRevision()`'s catch-all). The internal `WorkflowRevisionTombstonedError` never reaches either public operation boundary itself: `installWorkflowRevision()` and `installFencedSourceRevision()` both catch it and translate it to the public `WorkflowRevisionUnavailableError`, so the Conflict fault's `weftCode` a client sees is `WorkflowRevisionUnavailableError`, not the internal tombstone class.

### Fixed

- `countTeardownDeadLettersForRevision()` and `countLegacySingleSlotDeadLetter()` (`retained-recovery-record-count.ts`) previously caught and silently skipped an undecodable or malformed `TeardownDeadLetterRecord` (reachable whenever a peer or operator process scans a dead letter using a custom serializer tag this process has not registered, or one with a mismatched-type discriminant), under-counting and permitting `removeWorkflowRevision()` to remove a revision a real dead letter still durably pinned. Both now fail the whole scan closed on any undecodable or malformed record, matching the existing unguarded-`decode()` convention `nonterminal-revision-count.ts` already uses for `WorkflowState` records.
- The dead-letter reference count now also scans the legacy single-slot `wf-teardown-deadletter:` namespace for a pre-upgrade record with no history sibling — provable by the absence of its computed history key, since every write since the history namespace's introduction writes both keys together in the same batch. A record with a `revision` is matched exactly; one without is pinned conservatively against every queried revision, since its true revision cannot be determined. This scan buffers its entries before any `storage.get()` lookup rather than awaiting inside the loop, avoiding `IndexedDBStorage.scan()`'s known `TransactionInactiveError` risk (WFT-160).
- `finalizeRevisionRemoval()` (`catalog-removal.ts`) previously ignored a lost CAS from resolving its own tombstone, reporting `{ removed: true }` and dispatching `catalog:revision-removed` even when a concurrent boot-time sweep had already restored the entry. It now re-reads post-resolution durable state on a lost CAS and reports the truthful outcome either way.

### Persisted-data compatibility

- `TeardownDeadLetterRecord` (persisted under `wf-teardown-deadletter:<workflowId>`) gains an additive optional `revision?: string` field (WFT-21), set to the dead-lettering run's own `WorkflowState.revision`. `CURRENT_PERSISTED_DATA_SCHEMA_VERSION` is **not bumped** (stays `2`): a record written before this field existed, or for a legacy run with no persisted `WorkflowState.revision`, decodes with `revision: undefined` and is pinned conservatively toward EVERY queried revision of the matching workflow type in `countTeardownDeadLettersForRevision()`'s scan — since its true revision cannot be determined, under-counting it would risk permitting removal of a revision it still durably pins. This means an upgraded pre-revision-pinning run's dead letter can block removal of a revision it never actually ran, the deliberately conservative counterpart to `WorkflowState.revision`'s own legacy-record precedent from WFT-17.
- New durable namespace `wf-teardown-deadletter-history:<workflowId>:<workflowExecutionToken>` (WFT-21) — a new, independent key namespace, not a reshape of `wf-teardown-deadletter:<workflowId>`, which is untouched; `CURRENT_PERSISTED_DATA_SCHEMA_VERSION` is unaffected. Same purge-survival contract as `teardownDeadLetter`: excluded from the purge delete-set.
- `Checkpoint` (persisted under `wf:<workflowId>:ckpt`/`wf:<workflowId>:ckpt:<step>`) gains an additive optional `workflowExecutionToken?: string` field (WFT-21). `CURRENT_CHECKPOINT_SCHEMA_VERSION` is **not bumped** (stays `2`): mirrors this type's existing optional-field precedent (`accumulatedResultReplayWatermark`, `workerReplaySignatures`, `workerReplayFailures`) — `validateCheckpointShape()` already accepts the field's absence, validating it only when present. A checkpoint persisted before this field existed decodes with `workflowExecutionToken: undefined`; `replayTo()` omits `revision` for that case rather than falling back to a timing comparison.
- New additive, **PERMANENT** (never deleted) key prefix `catalog-removal-generation:<name>:<revision>` (WFT-21). `CURRENT_PERSISTED_DATA_SCHEMA_VERSION` is **not bumped** (stays `2`) — an entirely new key family no prior Weft version ever wrote, reshaping no existing persisted record's bytes or fields, so an older reader simply never looks for this prefix. Bytes are the decimal ASCII text of a monotonic count (`TextEncoder`-encoded), never a binary integer — compared byte-for-byte via `conditionalBatch` or round-tripped through its own encode/decode pair, with no sort or range-scan order implied.

### Known limitation, documented rather than fixed

- For an eager-registered workflow type specifically, recovery and a default (non-pinned) fork intentionally run whatever this process currently has eagerly registered, even when it differs from the run's own persisted `revision` — the pre-existing "eager is always ready regardless of pin" design from WFT-17/19. `revision` is stamped once, only at a run's genuine start/fork, and is never re-stamped by a later recovery, so a run recovered after this process redeploys to a new eager revision keeps reporting its old `revision` for checkpoints the new code goes on to produce. The `workflowExecutionToken` correlation above is still correct (it genuinely is the same execution); only the reported `revision` value itself can be stale for this specific eager-redeploy-during-recovery case (WFT-159).
