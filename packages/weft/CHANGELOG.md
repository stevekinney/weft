...
- Every execution entry point that can launch or resume a workflow — `start`, `startOrSignal`, `schedule`, `fork`, `resume`, `recoverAll` (and `Engine.create`, which calls it by default), a delayed-start timer fire, and bulk-retry — now awaits dynamic-source resolution through one shared internal helper (`resolveExecutableRegistration`) for a `registerSource()`-registered type before running any handler code (WFT-15/16, wiring the WFT-13/14 dynamic-source primitive into actual execution and recovery). An eagerly `register()`-ed type still resolves synchronously and never touches the dynamic-source machinery. Adds `engine.workflows.preload(name, revision, options?)` — a thin, documented alias for `engine.resolveWorkflowSource()` (identical single-flight, cancellation, and error contract) offered alongside the other `engine.workflows.*` catalog operations — plus the matching `weft.workflows.revisions.preload` REST (`POST /v1/registry/workflows/:name/preload`) and JSON-RPC operation. `weft.catalog.diagnostics`'s `WorkflowRevisionDiagnostics.source` field now reports bounded, per-`(name, revision)` load-state diagnostics (`kind`, `requestedRevision`, `state`, `loadDurationMs?`, `lastFailureCategory?`, `waiterCount`) for any name ever `registerSource()`-registered on the process. Four new process-local events — `workflow-source:load-started`, `workflow-source:load-ready`, `workflow-source:load-failed`, `workflow-source:load-cancelled` (`core/events/workflow-source-events.ts`) — observe a dynamic-source load's lifecycle. `recoverAll()` preloads every distinct dynamic-source type referenced by non-terminal state ONCE, before advancing any generator; a type whose load fails is classified unavailable and only its own runs fail, mirroring the existing version-mismatch sibling-isolation precedent. Cancellation/disposal is hardened so a cancelled `resolveWorkflowSource()` waiter never aborts a load another caller still needs, and engine disposal settles every pending waiter. This is a feature gate, not durable per-run revision pinning on its own — no persisted-data shape changes; WFT-17/18 (below) builds the actual per-run pin on top of it. Also fixes a correctness bug in the already-merged `resolveWorkflowSource()` cache-hit fast path (WFT-13/14): it skipped populating the local resolved-definition cache, which would have broken cross-process-restart recovery (a second process needs a LOCAL `WorkflowDefinition`, not just the durable manifest); the public `resolveWorkflowSource()` contract is unchanged.
...
- `Checkpoint` (persisted under `wf:<workflowId>:ckpt`/`wf:<workflowId>:ckpt:<step>`) gains an additive optional `workflowExecutionToken?: string` field (WFT-21, Codex review round 3, P2) — see the `engine.replayTo()` Added entry above. `CURRENT_CHECKPOINT_SCHEMA_VERSION` is **not bumped** (stays `2`): mirrors this type's own existing optional-field precedent (`accumulatedResultReplayWatermark`, `workerReplaySignatures`, `workerReplayFailures`) — `validateCheckpointShape()` already accepts the field's absence, validating it only when present (non-empty string). A checkpoint persisted before this field existed decodes with `workflowExecutionToken: undefined`; `replayTo()` falls back to its original `createdAt` comparison for that case.
