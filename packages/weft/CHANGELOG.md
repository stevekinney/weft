...
- **`Engine#getWorkflowActivityDefinition()`/`Engine#listWorkflowActivityDefinitions()` now correctly return `undefined`/`[]` for a `registerSource()`-registered type, resolved or not** (WFT-19), instead of possibly reflecting a stale or mismatched revision's activity metadata — a side effect of the mirror-write removal above, and a deliberate narrowing to eager-only scope for these two synchronous, type-only accessors, which have no running instance to resolve a specific revision against. Documented as a `Fixed` behavior narrowing, not a breaking change: these accessors never had defined multi-revision semantics to begin with. See [`getWorkflowActivityDefinition()`](documentation/reference/api-engine.md#getworkflowactivitydefinition).
