...
- Several revision-related engine faults that previously surfaced as an opaque `EngineFailure` (REST 500 / undeclared JSON-RPC failure) now map to their documented `Conflict` (409) fault: an explicit-revision fork's `VersionMismatchError`, a `WorkflowRevisionTombstonedError` surfaced by `weft.workflows.revisions.install()`/`preload()`, and a `WorkflowRevisionUnavailableError` surfaced by a dynamic-source load racing a concurrent removal (previously re-wrapped into a generic `DynamicWorkflowSourceUnavailableError` by `loadAndInstallSourceRevision()`'s catch-all).
