...
| Dynamic-source load diagnostics (`weft.catalog.diagnostics`) — the Registry → Dynamic workflow sources lookup panel, the only surface that requests them (WFT-116)                                                                                               | `scoped` — `system:read`                                                                                                                             |
| Workflow-catalog reads (`weft.workflows.revisions.get`/`.list`, `weft.workflows.active.get`) — the Registry → Revisions panel, the workflow detail header/overview's active-revision comparison badge, and the fork dialog's installed-revision picker (WFT-117) | `scoped` — `workflows:read`                                                                                                                          |
| Workflow-catalog writes (`weft.workflows.revisions.install`/`.activate`/`.preload`) — the Revisions panel's Activate/Refresh action and the Dynamic workflow sources panel's Preload action (WFT-116)                                                            | `scoped` — `workflows:admin`                                                                                                                         |
