...
- **Workflow catalog** (WFT-9..12, WFT-115): `weft.workflows.revisions.install`/`.activate` (`workflows:admin`), `weft.workflows.revisions.get`/`.list`, `weft.workflows.active.get` (`workflows:read`) — JSON-RPC only, no client REST binding is generated for any of the five (`CLIENT_REST_OPERATION_BINDINGS` lists a single unrelated operation, verified against the installed package); `.activate` returns `{ applied: true, pointer }` or refuses with a `Conflict` fault whose `data.compatibilityReasons`/`data.currentGeneration` carry the bounded `WorkflowCompatibilityReason` list or the stale-generation signal.
