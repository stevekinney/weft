...
List (human-readable spec, next/last fire, missed count red >0, pause/resume/cancel/edit), Detail (spec with next-5-fires preview, **overlap policy with one-sentence consequence text** — the four-policy table from the source doc is retained verbatim, jitter, backfill, queued runs when overlap=queue, history of fired runs, **revision policy** — `active-at-fire` or `Pinned (rev X)`, WFT-20/WFT-117), Create/Edit `Drawer` (CronField (C3) / interval toggle, overlap radio with inline consequences, backfill warning, start-paused, **revision-policy radio** — WFT-117: active-at-fire vs. pinned, with a warning that saving while pinned always re-captures the currently active revision; unlike overlap/jitter/backfill/description, the revision policy IS editable after creation — `ScheduleUpdateOptions` accepts it, and the console's edit-mode field-scope note says so). API: `/api/v1/schedules` CRUD + `pause`/`resume`. `schedule:fired`/missed-fire events arrive on the fleet feed for liveness.
