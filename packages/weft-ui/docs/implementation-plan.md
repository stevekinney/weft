...
- **TanStack Query** owns all server state. Query keys encode full fetch parameters: `['workflows','list',filter]`, `['workflows','detail',id]`, `['workflows','events',id,cursor]`, `['workflows','aggregate',groupBy,filter]`, `['schedules','list',filter]`, `['workers','list']`, `['queues','list']`, `['diagnostics']`, `['reviews','list',filter]`, `['registry']`, `['retention']`, `['metrics']`, `['principal']`, `['catalog','revisions',name]`, `['catalog','active',name]` (WFT-115, reused verbatim by the workflow detail header/overview's active-revision comparison badge and the fork dialog's revision picker — WFT-117, no new query key needed).
...
- **List**: `DataTable` + `FacetedFilterBar` bound to the URL filter serializer; search-attribute query-builder rows (C4); Live toggle via fleet feed (status patches on visible rows, "+N new" for matching arrivals); bulk-selection bar → Tier-3 flow driven by the server's **dry-run/confirmation-token** protocol (preview count comes from the dry run — the retry-failed dialog's preview additionally notes that each retried run keeps its own persisted revision, WFT-117). Start Workflow wizard (`Steps` + `SchemaForm` from `/api/v1/registry` `inputSchema`, advanced options, review step showing the type's active revision the run will start against, 409 idempotency-conflict treatment with spent-key explanation).
- **Revision bindings** (WFT-117): the list's Revision column shows each row's own persisted `WorkflowSummary.revision` (an explicit "Unpinned" badge for a pre-revision-pinning record — never a blank cell); the detail header and Overview tab's Revision row additionally compare it against the type's currently active revision (`weft.workflows.active.get`) with an icon+text treatment (active / differs from active / active revision unknown — never color alone); the Children tab and Lineage panel's forked-from row and children preview show each run's own revision; the Lineage panel's continuation chain shows this run's revision alongside "a start-new replacement always resolves against whichever revision is active at the moment it replaces the prior run." Every eager-registered-type revision comparison carries the WFT-159 hedge (recovery and a default fork always run this process's currently loaded code, even if it differs from the displayed revision).
- **Detail**: header (copyable ID, status incl. `finalizing` / `cancelled (finalizer failed)` sub-states, tags, deadline countdown; contextual actions per status × scope; Signal/Update/Query as three distinct labeled buttons with semantics tooltips). Tabs: **Overview** (`DescriptionList` + `PayloadInspector` for input/result/error; failure-category badge + plain-language explanation), **Timeline** (`RunStepTimeline` from `GET …/timeline`; branch/saga/finalizer rendering per C1; step expansion with per-attempt errors, heartbeat, retry policy; quick filters; step-range pagination past ~500 with jump-to-step), **Events** (`EventStreamViewer`, live tail, cursor-resumed), **Logs** (`ctx.log` in execution order with the replay re-emit note), **Checkpoints** (replay/fork actions — `GET …/checkpoints`, `GET …/replay/:step`, `POST …/fork`; the replay view shows the replayed checkpoint's own `WorkflowReplay.revision` or an explicit "not attributable" explanation when absent; the fork dialog shows the default retention line — "Retains rev X" — plus an opt-in "Fork a different revision" disclosure listing installed revisions via `weft.workflows.revisions.list`, WFT-117), **Signals** / **Updates** (history + send forms; update result long-polled via `GET /api/v1/updates/:updateId` with pending countdown), **Children** (child table; child lanes also visible in the timeline).
...
List (human-readable spec, next/last fire, missed count red >0, pause/resume/cancel/edit), Detail (spec with next-5-fires preview, **overlap policy with one-sentence consequence text** — the four-policy table from the source doc is retained verbatim, jitter, backfill, queued runs when overlap=queue, history of fired runs, **revision policy** — `active-at-fire` or `Pinned (rev X)`, WFT-20/WFT-117), Create/Edit `Drawer` (CronField (C3) / interval toggle, overlap radio with inline consequences, backfill warning, start-paused, **revision-policy radio** — WFT-117: active-at-fire vs. pinned, with a warning shown only when the edit SWITCHES the policy to pinned that saving re-captures whichever revision is active right now (an already-pinned schedule's cadence-only edit preserves its existing captured pin unchanged, never re-resolving it); unlike overlap/jitter/backfill/description, the revision policy IS editable after creation — `ScheduleUpdateOptions` accepts it, and the console's edit-mode field-scope note says so). API: `/api/v1/schedules` CRUD + `pause`/`resume`. `schedule:fired`/missed-fire events arrive on the fleet feed for liveness.
...
- **Registry** (`weft.system.registry`, `RegistrySnapshot` v2 — WFT-6): workflow definitions with revision identity (revision, manifest version, contract hash), expandable readable `inputSchema`/`outputSchema` trees (`Tree`), the full contract surface (signal/update/query/activity/finalizer schemas, sourced for real as of WFT-115), activity definitions with retry policy and timeout; 3-step onboarding empty state. **Revisions panel** (WFT-115): every durably installed revision of the selected workflow (`weft.workflows.revisions.list`) against the currently active pointer (`weft.workflows.active.get`), and a `workflows:admin`-gated Activate action (`weft.workflows.revisions.activate`) whose outcome — applied, or refused with the full bounded `WorkflowCompatibilityReason` list — doubles as the compatibility preview; there is no separate read-only preview operation. **Dynamic workflow sources panel** (WFT-116): per-`(name, revision)` load diagnostics from `weft.catalog.diagnostics` (`system:read`) — source kind, requested revision, idle/loading/ready/failed/cancelled state, load duration, waiter count, last bounded failure category, and current catalog installation — reached through a `(name, revision)` lookup above the registered-definitions table, plus a `workflows:admin`-gated Preload action (`weft.workflows.revisions.preload`). That lookup is the SOLE diagnostics surface: the Revisions panel deliberately does not show them per row, because every row it can reach is an eager registration whose `source` block is always absent, and each call would run `countWorkflowRevisionReferences()`'s storage scans for that constant answer. The lookup is free text rather than a picker because Weft publishes no operation that enumerates registered sources, and `weft.system.registry` is built from the engine's eager registrations only, so a `registerSource()` workflow never appears in the table above — not even after a preload installs one of its revisions durably (filed upstream as WFT-165). No Cancel control is offered: Weft exposes no operation that cancels an in-flight load, and aborting the console's own request would not touch the engine's single-flight one.
...
- **Workflow catalog** (WFT-9..12, WFT-115): `weft.workflows.revisions.install`/`.activate`/`.preload` (`workflows:admin`), `weft.workflows.revisions.get`/`.list`, `weft.workflows.active.get` (`workflows:read`) — JSON-RPC only, no client REST binding is generated for any of these six (`CLIENT_REST_OPERATION_BINDINGS` lists a single unrelated operation, verified against the installed package). `.preload` installs one `registerSource()`-registered revision into the catalog (non-destructive, like `.install`), loading and validating the module only when the catalog does not already hold that revision — a durable cache hit returns the existing entry without invoking the loader at all, so a successful preload can legitimately leave the source's load state `idle`. Consumed by System → Registry → Dynamic workflow sources (WFT-116). `.activate` returns `{ applied: true, pointer }` or refuses with a `Conflict` fault whose `data.compatibilityReasons`/`data.currentGeneration` carry the bounded `WorkflowCompatibilityReason` list or the stale-generation signal.
