...
- `ctx.race()` / `ctx.all()` branch changes must preserve sleep and wait-signal support, nested coordinator propagation, duplicate signal-name rejection across the coordination tree, non-destructive signal losers, and deferred signal consumption before checkpointing. Pin `ctx.speculate` separately because it drives nested coordinators through a different top-level path.
