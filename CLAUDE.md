...
- The REST client contract is the current flat body `{ error, weftCode?, data? }`; do not restore parsing or documentation for the retired nested error envelope. `POST /v1/recover` keeps its conflict details under `data`.
- Direct `handleRequest()` hosts may inject a live `WorkerRegistry` and `TaskQueue` through `HandlerOptions`; keep those instances aligned with the default operation registry so worker, queue, and task-diagnostics routes observe the same live state.
...
- CLI `SIGINT` and `SIGTERM` must use one memoized shutdown promise. Preserve the order `stopServer` → best-effort run-lock removal → storage disposal → success exit, and ensure a second signal cannot start a second cleanup sequence or change the first result.
- CLI workflow modules must export canonical `WorkflowDefinition` values (plus `ActivityDefinition` values where needed); register each definition by its own runtime name, preserve names such as `__proto__` and `toString`, and reject removed bare-handler registrations with the builder diagnostic. Do not reintroduce wrapper or compatibility registration layers.
...
- When a test-only seam must live on a production class or module, keep it inline in that existing emitted module. Do not extract it into a standalone `src/**/*.ts` helper, which ships, or rename a production-imported target to `.test-support.ts`, which leaves a dangling `dist/` import; run `bun run build` to prove the emitted graph.
- Service Worker test message handlers that pass async work to `event.waitUntil()` must catch rejections and post `{ error }` through the reply port. Keep an inner readiness timeout strictly shorter than the outer message timeout and clear its timer and waiter on every settlement path so CI receives the causal diagnostic instead of a generic transport timeout.
...
- Line-keyed coverage allowances must be re-derived from fresh LCOV after source movement and use `requireUncoveredLines: true` when they target exact misses. Add parser coverage for covered-line drift; never let an allowance silently subtract a covered or unrelated line.
