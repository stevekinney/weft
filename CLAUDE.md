...
- RemoteWorker SDK changes must preserve the required `workflows` map, qualified `${workflowType}.${activityName}` advertisement, and key/name validation. Do not reintroduce the removed flat `activities` alias; `LongPollWorker.activities` and the wire `register.activities` array are separate contracts.
