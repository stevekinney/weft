...
- Per-run workflow `services` are inline-only host capabilities, not durable state. Starting with `services` must persist only the presence marker, reject Worker execution mode, re-provide services through `resolveWorkflowServices` on running recovery, delayed-start recovery, and scheduled occurrences, pass recovered `launchOptions` and durable `schedule-run` metadata into the resolver when available, fail only the affected run when unavailable, and sweep the marker plus schedule-run metadata on terminal cleanup, purge, and retention. Keep `WorkflowServicesResolverLaunchOptions` and `WorkflowServicesResolverScheduleInfo` root-exported while `WorkflowServicesResolverInfo` exposes those fields.
