...
- `ctx.log` changes must preserve checkpoint-aware suppression without consuming a durable step: envelope fields stay engine-owned, attributes stay nested, worker mode logs to the worker console, and logs at uncached live frontiers may re-emit after recovery.
